...
function TextLiveInterview({ jobId, sessionId, session, job, company }: SharedProps) {
  ...
  }, [transcript, streamingContent, thinking]);

  /**
   * Call the streaming API route and pipe tokens into state.
   * When the stream finishes, commit the completed turn to `transcript`.
   */
  async function streamTurn(candidateMessage: string | null) {
    ...
    const res = await fetch("/api/interview-turn-stream", {
      ...
      body: JSON.stringify({ session_id: sessionId, candidate_message: candidateMessage }),
    });
    ...
    // Read the plain-text stream token by token
    const reader = res.body.getReader();
    const decoder = new TextDecoder();
    let accumulated = "";

    setStreamingContent(""); // show the growing bubble immediately
    ...
    // Stream complete — move from streaming bubble into transcript
    const completedTurn: Turn = {
      role: "interviewer",
      content: accumulated,
      timestamp: new Date().toISOString(),
    };
    setTranscript((prev) => [...prev, completedTurn]);
  ...
  async function send() {
    ...
    if (!msg || thinking || streamingContent !== null) return;
    setInput("");
    ...
    <div className="flex h-[calc(100vh-7rem)] flex-col">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto py-6">
        <div className="mx-auto max-w-3xl space-y-5">
          ...
          {/* Streaming bubble — grows token by token while response is arriving */}
          {streamingContent !== null && (
            <div className="flex gap-3">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/15 text-xs font-medium text-primary">
                {initials(persona.title)}
              </div>
              <div className="max-w-[80%] whitespace-pre-wrap rounded-2xl border border-border bg-card px-4 py-3 text-sm leading-relaxed">
                {streamingContent}
                <span className="ml-0.5 inline-block h-3.5 w-0.5 animate-pulse bg-muted-foreground align-middle" />
              </div>
            </div>
          )}
      ...
      <div className="border-t border-border pt-4">
        <div className="mx-auto max-w-3xl">
          ...
          <div className="mt-2 flex justify-between text-xs text-muted-foreground">
            ...
            <Button size="sm" onClick={send} disabled={thinking || streamingContent !== null || !input.trim()}>
              Send
            </Button>
          </div>
        </div>
