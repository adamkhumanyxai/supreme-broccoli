...
import { Loader2, Sparkles, Search, FileText, Download, Wand2, Minimize2, Maximize2, MessagesSquare, Lightbulb, Pencil, Check, X, Trash2, HelpCircle } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
/** Inline edit/preview toolbar for a single drafting section. */
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <TooltipProvider delayDuration={200}>
          <Tooltip>
            ...
            <TooltipContent side="left" className="space-y-1 p-3 text-xs">
              {CHEATSHEET.map(({ syntax, label }) => (
                <div key={label} className="flex items-center gap-2">
                  <code className="rounded bg-muted px-1 py-0.5 font-mono">{syntax}</code>
      ...
      {mode === "edit" ? (
        <Textarea
          ...
          className={`font-mono text-sm ${minHeight}`}
          disabled={disabled}
        />
      ) : (
        <div
          className={[
            minHeight,
            "rounded-md border border-border bg-muted/30 px-4 py-3 overflow-auto",
            theme === "dark"
        ...
        >
          {section.content ? (
            ...
            <p className="text-sm text-muted-foreground italic not-prose">
              Nothing drafted yet. Switch to Edit or run an AI action.
            </p>
          )}
        </div>
      )}
...
function DraftingTab({
  ...
  // Per-section edit/preview mode, keyed by section id
  const [sectionModes, setSectionModes] = useState<Record<string, "edit" | "preview">>({});
  ...
  function getSectionMode(id: string): "edit" | "preview" {
    return sectionModes[id] ?? "edit";
  }

  function setSectionMode(id: string, mode: "edit" | "preview") {
    setSectionModes((prev) => ({ ...prev, [id]: mode }));
  }
  ...
  async function runAction(
    ...
    try {
      ...
      setOutline(next);
      // Auto-switch to preview so the user sees rendered output immediately
      setSectionMode(id, "preview");
      onUpdate();
      ...
      {outline.map((s) => {
        ...
        return (
          <div key={s.id} className="editorial-card space-y-3 p-6">
            ...
            <SectionEditor
              section={s}
              mode={getSectionMode(s.id)}
              onModeChange={(m) => setSectionMode(s.id, m)}
              onContentChange={(content) => {
                const next = outline.map((x) => (x.id === s.id ? { ...x, content } : x));
                setOutline(next);
              }}
